...
class AlienInvasion:
    ...
    def _check_bullet_alien_collisions(self):
        ...
        if not self.aliens:
            ...
            self.stats.level += 1
            # self.sb.prep_level() 
            self._check_level_provide_bullets(self,self.sb)

    def _update_aliens(self):
        """检查是否有外星人位于屏幕边缘，并更新整个外星舰队位置"""
    ...
    def _write_high_score(self):  
        ...
        Note.write(str(self.stats.high_score) + '\n')
        Note.close()
    ...
    def _check_level_provide_bullets(self_settings,stats,sb):
        """检查等级是否足够高，可以提供大额子弹"""
        dream_level_1 = 2
        dream_level_2 = 3
        dream_level_3 = 4
        ...
        if stats.level >= dream_level_1:
            self_settings.bullet_width = 15
        elif stats.level >= dream_level_2:
            self_settings.bullets_allowed = 5
            self_settings.bullet_width = 20
        elif stats.level >= dream_level_3:
            self_settings.bullet_speed_factor = 1.5 * self_settings.bullet_speed_factor
        sb.prep_level()
